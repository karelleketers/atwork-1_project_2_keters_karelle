...
(()=>{
    const prog = {
        ...
        cacheElements(){
            ...
            this.$showGrid = document.querySelector('.prog__sifted__wrapper');
            this.$showList = document.querySelector('.prog__sifted__listed');
            /* this.$list = document.querySelector('.prog__sifted__listed'); */
            this.$list = document.querySelector('.prog__sifted__wrapper')
            ...
            getJSON(GENTSE_FEESTEN_API, 
                (data) => {
                    ...
                    data.forEach((event, index) => {
                        circus = false;
                        day21 = false;
                        if (event.day == 20) {day21 = true}
                        event.category.forEach(el => {if (el == 'Circus') {circus=true}})
                        if (event.image && circus && day21) {
                            tempStr += `<div class="prog__sifted__header"><h3 class="prog__sifted__title" id="circus">Circus</h3><a href="#top" class="to__top"><img class="to__top__icon" src="./media/chevron-arrow-down--bl.svg" alt="arrow--up" /></a></div><section class="section--top prog__sifted__cat circus"><a href="./detail.html" class="article__link top__article__link"><article class="top__article"><figure class="article__figure top__article__figure"><div class="article__container top__article__container"><img class="article__image top__article__image" src=${event.image.full} alt="event_image"/></div><figcaption class="top__article__figcaption">&nbsp;${event.start} u.<span class="top__article__figcap__span"></span></figcaption></figure><div class="article__titles"><h1 class="top__article__title">${event.title}</h1><h2 class="top__article__subtitle">${event.location}</h2></div></article></a></section>`
                        }});
